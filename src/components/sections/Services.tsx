        ...
        <div className="mt-14 grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {services.map((s, i) => (
            <motion.div
              ...
              className="group relative glass-dark rounded-3xl p-6 hover:bg-white/10 transition-smooth cursor-pointer"
            >
              <div className="h-12 w-12 rounded-2xl bg-gradient-primary grid place-items-center mb-6 group-hover:scale-110 transition-smooth">
                <s.icon className="h-6 w-6 text-white" />
              </div>
              <h3 className="font-display text-xl font-bold">{s.title}</h3>
              <p className="mt-3 text-sm text-white/65 leading-relaxed">{s.desc}</p>
              <div className="mt-6 flex justify-end">
                <span className="h-10 w-10 rounded-full bg-white/10 grid place-items-center group-hover:bg-primary group-hover:rotate-45 transition-smooth">
